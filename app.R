...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Further Analysis",tabName = "plots")
...
The dashboard provides an interface where a user can navigate and take quick actions to the various sections of the application for a detailed view. The dashboard and its entire 
  ...
  The data visualization choice is in a manner that will inform a user on The data visualization choice is in a manner that will inform a user on
  disadvantaged populations, that is, groups that are less likely to have access to adequate and nutritious food.  
"),
              h2("Key Findings"),
              p("The data is a secondary data sourced from the Annual Household Income and Expenditure Survey, 2022, by the Ghana Statistical Service"),
...
•	Comparing marital status with food insecurity, the widowed group are more severely food insecure than any other marital status, followed by the divorced group.
")
